...
def detect(opt):
    ...
    ZONE_CH5 = [[260, 576], [260, 440], [380, 440], [480, 480], [680, 520], [680, 576]] #for bottom center points
    ZONE_CH5_IN = [[260, 580], [260, 450], [400, 450], [530, 500], [530, 580]] #for center point
    roi_entrance = np.array(ZONE_CH5).reshape((-1,1,2)).astype(np.int32)     
    roi_in = np.array(ZONE_CH5_IN).reshape((-1,1,2)).astype(np.int32)     
            ...
            for i, det in enumerate(pred):  # detections per image
                ...
                if det is not None and len(det):
                    ...
                    if len(xywh_bboxs) > 0 and len(confs) > 0 and len(obj_cls) > 0:
                        ...
                        for idx in range(len(xywh_bboxs)):
                            ...
                            if not (
                                (dist_a2entrance >= 0 and dist_d2entrance >= 0) and \
                                (dist_c2entrance <= -20 and dist_b2entrance <= -20) and \
                                (box_diagonal >= 60 and box_diagonal <= 500)
                                ):
                    ...
                    if (len(ids_emerging) > 0):
                        ...
                        for idx in range(len(ids_emerging)):
                            ...
                            x1, y1 = bbox_xyxy[identities==key, 0], bbox_xyxy[identities==key, 1]
                            x2, y2 = bbox_xyxy[identities==key, 2], bbox_xyxy[identities==key, 3]
                            if key not in list(passenger_in): passenger_in[key] = deque(maxlen=1)
                            passenger_in[key]=(frame_idx, int((x1+x2)/2), int((y1+y2)/2))
                    ...
                    if (len(ids_gone) > 0):
                        ...
                        for idx in range(len(ids_gone)):
                            ...
                            x1, y1 = bbox_xyxy_prev[identities_prev==key, 0], bbox_xyxy_prev[identities_prev==key, 1]
                            x2, y2 = bbox_xyxy_prev[identities_prev==key, 2], bbox_xyxy_prev[identities_prev==key, 3]
                            if key not in list(passenger_out): passenger_out[key] = deque(maxlen=1)                            
                            passenger_out[key]=(frame_idx, int((x1+x2)/2), int((y1+y2)/2))

                    if (len(ids_gone) > 0):
                        for idx in range(len(ids_gone)):                    
                            key = int(ids_gone[idx])
                            if key in list(passenger_in) and key in list(passenger_out):
                                (t1, xtn1, ytn1) = passenger_in[key]
                                (t2, xtn2, ytn2) = passenger_out[key]
                                displacement = np.sqrt((xtn2-xtn1)*(xtn2-xtn1)+(ytn2-ytn1)*(ytn2-ytn1))
                                time_elapsed = t2 - t1
                                if time_elapsed >= 2*step_frames:
                                    if displacement >= 0:
                                        if xtn2 - xtn1 <= 0: 
                                            ppl_in += 1 
                                            ppl_count += 1
                                ...
                                pta = [int(x1), int(y2)]
                                ptd = [int(x2), int(y2)]
                                pt0 = [int(x1*2/3+x2/3), int(y2)]
                                dist_a2in = cv2.pointPolygonTest(roi_in, pta, True) 
                                dist_d2in = cv2.pointPolygonTest(roi_in, ptd, True)                             
                                dist_02in = cv2.pointPolygonTest(roi_in, pt0, True) 
                                if ((dist_a2in <= 0 and dist_d2in >= 0) or \
                                    (dist_a2in >= 0 and dist_d2in >= 0) or \
                                    (dist_a2in >= 0 and dist_d2in <= 0 and dist_02in >= 0)): passenger_in += 1
                                else: passenger_out += 1
                                '''
                ...
                if show_vid:
                    if show_roi: 
                        ...
                        #cv2.polylines(im0, [roi_in], isClosed = True, color = (0, 0, 255), thickness = 2)                                                                         
